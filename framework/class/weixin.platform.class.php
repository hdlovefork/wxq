...
define('ACCOUNT_PLATFORM_API_ACCESSTOKEN2', 'https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid=%s&secret=%s');
...
define('ACCOUNT_PLATFORM_API_OAUTH_CODE2', 'https://open.weixin.qq.com/connect/oauth2/authorize?appid=%s&redirect_uri=%s&response_type=code&scope=snsapi_base&state=%s#wechat_redirect');
...
define('ACCOUNT_PLATFORM_API_OAUTH_INFO2', 'https://api.weixin.qq.com/sns/oauth2/access_token?appid=%s&secret=%s&code=%s&grant_type=authorization_code');
...
//            $getTokenUrl=sprintf(ACCOUNT_PLATFORM_API_ACCESSTOKEN2,
//                $this->account['account_appid'],
//                $this->account['secret']);
//			$response = $this->request($getTokenUrl);
//			if (is_error($response)) {
//				$errormsg = $this->error_code($response['errno'], $response['message']);
//				return error($response['errno'], $errormsg);
//			}
...
//			cache_write('account:component:assesstoken', $accesstoken);
	...
	public function getOauthCodeUrl($callback, $state = '') {
		return sprintf(ACCOUNT_PLATFORM_API_OAUTH_CODE2, $this->account['account_appid'], $callback, $state);
	}
...
//		$apiurl = sprintf(ACCOUNT_PLATFORM_API_OAUTH_INFO2, $this->account['account_appid'], $this->account, $code);
		$apiurl = sprintf(ACCOUNT_PLATFORM_API_OAUTH_INFO2, $this->account['account_appid'], $this->account['secret'], $code);
