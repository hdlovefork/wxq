...
//        print_r($menus);
    ...
    public function regions()
    {
        ...
        $regions = pdo_getall('xcommunity_region', $condition, array('id', 'title'));
        return $regions;
    }
    ...
    {
        global $_W;
        $credit = pdo_fetch("SELECT * FROM" . tablename('xcommunity_set') . "WHERE uniacid=:uniacid", array(':uniacid' => $_W['uniacid']));
        return $credit;
    }
    ...
    {
        global $_GPC, $_W;
        $member = pdo_fetch("SELECT * FROM" . tablename('xcommunity_member') . "WHERE openid='{$_W['fans']['from_user']}' AND weid=:uniacid AND enable = 1", array(':uniacid' => $_W['uniacid']));
        if (empty($member)) {
...
//        return $member;
        ...
        //先通过openid查询会员，如果查不到再通过手机号码查询
        $member = pdo_fetch("SELECT * FROM" . tablename('xcommunity_member') . "WHERE openid=:openid AND weid=:uniacid AND enable = 1", array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
        if (empty($member)) {
            $member = pdo_fetch("SELECT * FROM" . tablename('xcommunity_member') . "WHERE mobile=:openid AND weid=:uniacid AND enable = 1", array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
        }
        return $member;
    }
...
//    print_r($params);exit();
        ...
        if ($params['type'] == 'delivery') {
            ...
            if ($order['type'] == 'shopping') {
                ...
                $credit = $params['fee'] * $c['shop_credit'];
                ...
                $credit = $params['fee'] * $c['business_credit'];
        ...
        if ($params['result'] == 'success' && $params['from'] == 'notify' || $params['type'] == 'credit') {
            ...
            if ($order['type'] == 'business') {
                ...
                $credit = $params['fee'] * $c['cost_credit'];
        ...
        if ($params['from'] == 'return') {
            $url = 'http://' . $_SERVER['HTTP_HOST'] . "/app/index.php?i={$_W['uniacid']}&c=entry&do=home&m=xfeng_community";
    ...
    public function doMobileXqsys()
    {
        global $_W;
        include $this->template($this->xqtpl('xqsys'));
    }

    public function auth($apikey, $deviceid, $lockid)
    {
        global $_W;
        load()->func('communication');
        $domain = $_SERVER['SERVER_NAME'];
//        if($deviceid&&$lockid&&$apikey){
//            $resp = ihttp_request('http://cloud.we7xq.com/addons/bull_manage/api.php', array('apikey' => $apikey,'deviceid' => $deviceid,'lockid' => $lockid,'domain' => $domain),null,1);
//            $resp = @json_decode($resp['content'], true);
//            return $resp['content'];
//        }
        if ($deviceid) {
            $resp = ihttp_request('http://cloud.we7xq.com/addons/bull_manage/api.php', array('deviceid' => $deviceid, 'domain' => $domain, 'type' => 'auth'), null, 1);
            $resp = @json_decode($resp['content'], true);
