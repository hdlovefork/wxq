 ...
 */
	...
	if($op == 'list'){
		...
		$member = $this->changemember();
		if($member['type']){
			$address = pdo_get('xcommunity_member_address',array('openid' => $_W['fans']['from_user'],'regionid' => $member['regionid']),array('realname','telephone','address'));
		}
        ...
        if($this->module['config']['xq_pay']&&$this->module['config']['xq_wechat']){
            //借用微信支付
            $api = pdo_get('xcommunity_pay_api',array('uid' => $order['uid'],'paytype' => 2),array('pay'));
