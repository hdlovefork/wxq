...
if ($op == 'add') {
    ...
    if ($_W['isajax']) {
        ...
        //微信通知
        $notices = pdo_fetchall("SELECT * FROM" . tablename('xcommunity_notice') . "WHERE uniacid=:uniacid AND enable =3 ", array(':uniacid' => $_W['uniacid']));
        foreach ($notices as $key => $value) {
            if ($value['type'] == 1 || $value['type'] == 3) {
                $regions = unserialize($value['regionid']);
                $cids = explode(',',$value['cid']);
                if (@in_array($member['regionid'], $regions) && @in_array(intval($_GPC['category']), $cids)) {
                    // $list = $notice;
    ...
    include $this->template($this->xqtpl('homemaking/add'));
}
elseif ($op == 'my') {
    if ($_W['isajax']) {
    ...
    include $this->template($this->xqtpl('homemaking/my'));
}
elseif ($op == 'delete') {
    $id = intval($_GPC['id']);
