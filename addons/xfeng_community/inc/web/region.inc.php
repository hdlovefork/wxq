 ...
 */
	...
	if ($op == 'add') {
		if (empty($id)) {
			...
			if ($user) {
				...
				if($user['uuid']&&$user['uuid']!=1){
                    $u = pdo_fetch("SELECT * FROM".tablename("xcommunity_users")."WHERE uniacid=:uniacid AND uid=:uid",array(":uniacid" => $_W['uniacid'],":uid" => $user['uuid']));
                    $maxaccount = $u['maxaccount'];
                }else{
		...
		$sql    = "select * from ".tablename("xcommunity_open_log")."as l left join".tablename("xcommunity_region")."as r on l.regionid = r.id where l.uniacid=:uniacid AND l.regionid=:regionid order by createtime desc LIMIT ".($pindex - 1) * $psize.','.$psize;
		$list   = pdo_fetchall($sql,$params);
