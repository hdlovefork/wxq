...
load()->app('common');//include IA_ROOT . '/app/common/' . $name . '.func.php';
load()->app('template');//include  IA_ROOT . '/framework/model/' . $name . '.mod.php';
load()->model('app');//include IA_ROOT . '/framework/model/' . $name . '.mod.php';
...
$init = IA_ROOT . "/app/source/{$controller}/__init.php";//根据$_GPC的m,do参数初始化$entry()数组，m->module,do->do
if(is_file($init)) {
	require $init;
}
